...
exports.getOptions = function (model: any): any {
	var bookmarks = {};

	if (model.uid && model.members.length > 0) {
		model.members.forEach(function (item) {
			bookmarks[item.uid] = shared.createHtmlId(item.uid);
		});
...
function updateChildrenNM(model: any): void {
	if (model.members && model.members.length > 0) {
		...
		for (var k = 0; k < model.members.length; k++) {
			...
			model.members[k].htmlId = shared.createHtmlId(model.members[k].uid);
