...
internal class InBuiltPackage : Package
{
    private static readonly EnumerationOptions s_enumerationOptions = new() { RecurseSubdirectories = true };
    private static readonly string s_templatePath = Path.Combine(AppContext.BaseDirectory, "data", "templates");

    public InBuiltPackage(string inBuiltName)
    {
        var fullPath = Path.Combine(s_templatePath, inBuiltName);
        if (!Directory.Exists(fullPath))
        {
            throw new DirectoryNotFoundException("Template doesn't exist!");
        }

        _directory = new PathString(fullPath);
    }
